...
installed=0
managed_projections=0
for root in "${roots[@]}"; do
  mkdir -p "$root"
  target="$root/hyperconsciousness"
  install_mode=link
  case "$root" in
  ...
  if test "$install_mode" = link; then
    if test "$(cd "$root" && pwd -P)/hyperconsciousness" = "$skill_dir"; then
      :
    elif test -L "$target"; then
      resolved_target=$(cd "$target" && pwd -P)
      if test "$resolved_target" != "$skill_dir"; then
        grep -q '^name: hyperconsciousness$' "$target/SKILL.md" 2>/dev/null || {
    ...
    if test -L "$target"; then
      ...
      case "$resolved_target" in
        "$HOME/.brainmesh-workspaces/"*/screenpipe-skills/hyperconsciousness)
          managed_projections=$((managed_projections + 1))
          installed=$((installed + 1))
          continue
          ;;
      esac
      test "$resolved_target" = "$skill_dir" || {
        printf 'refusing unrelated link: %s\n' "$target" >&2
        exit 1
      }
      unlink "$target"
      ...
      if test -f "$target/SKILL.md"; then
        grep -q '^name: hyperconsciousness$' "$target/SKILL.md" || {
          printf 'refusing unrelated target: %s\n' "$target" >&2
          exit 1
        }
...
printf 'managed_projection_roots=%s\n' "$managed_projections"
printf 'skill_sha256='
